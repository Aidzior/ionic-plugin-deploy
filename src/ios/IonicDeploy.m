...
- (void) pluginInitialize {
    ...
    self.cordova_js_resource = [self.cordova_js_resource stringByReplacingOccurrencesOfString:@"?" withString:@"%3F"];
    ...
    if (![uuid isEqualToString:@""] && !self.ignore_deploy && ![uuid isEqualToString:ignore]) {
        dispatch_async(self.serialQueue, ^{
            if ( uuid != nil && ![self.currentUUID isEqualToString: uuid] ) {
                ...
                newConfigReference = [newConfigReference stringByReplacingOccurrencesOfString:@"?" withString:@"%3F"];
